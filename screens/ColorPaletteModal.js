...
const ColorPaletteModal = ({ navigation }) => {
  ...
  const handleSubmit = useCallback(() => {
    if (!newPalette) {
        ...
        Alert.alert('Please add at least three colors');
    }
    else {
...
}, [newPalette, selectedColors]);

const handleValueChange = useCallback((value, color) => {
    if (value === true) {
        setSelectedColors(colors => [...colors, color]);
    } else {
        setSelectedColors(colors => colors.filter(selectedColor => color.colorName !== selectedColor.color))
    }
}, []);

  return (
    <SafeAreaView style={styles.container}>
      <Text style={styles.headerText}>Name of your color palette</Text>
      <TextInput
        style={styles.textBox}
        placeholder="Palette name"
        onChangeText={setNewPalette}
        value={newPalette}
      />
      <FlatList 
      data={COLORS}
